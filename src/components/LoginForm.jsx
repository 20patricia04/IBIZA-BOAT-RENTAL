...
function LoginForm() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Trimitem datele către Auth0 pentru a obține Token JWT
      ...
      console.log('Token salvat:', localStorage.getItem('authToken'), sessionStorage.getItem('authToken'));
console.log('Email salvat:', localStorage.getItem('userEmail'), sessionStorage.getItem('userEmail'));

      //Trimitem tokenul la backend ca să înregistrăm utilizatorul
      await axios.post(
      ...
      // După ce userul e creat sau există deja, cerem datele lui
      ...
      if (rememberMe) {
        localStorage.setItem('userId', userId); // ✅ Ghilimele corecte
