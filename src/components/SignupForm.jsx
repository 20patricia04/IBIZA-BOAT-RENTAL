...
function SignupForm() {
  ...
  const validatePassword = (password) => {
    //Reguli minimale: 8 caractere, 1 majusculă, 1 cifră
    const minLength = 8;
  ...
  const validateEmail = (email) => {
    // Validare structura email: text@domeniu.extensie
  ...
  const handleSignup = async (e) => {
    ...
    try {
      // Cerere directă către Auth0 pentru crearea contului
      ...
      console.error("Eroare la creare cont:", error.response?.data || error.message);
      // Tratarea erorilor
      if (error.response) {
